    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <button
        ...
        className={`relative text-left overflow-hidden rounded-2xl p-5 border transition-all group cursor-pointer ${
          currentTab === 'all'
            ? 'bg-blue-950/40 border-blue-500/50 shadow-lg shadow-blue-500/15'
      ...
      <button
        ...
        className={`relative text-left overflow-hidden rounded-2xl p-5 border transition-all group cursor-pointer ${
          currentTab === 'hot'
            ? 'bg-amber-950/50 border-amber-500/60 shadow-lg shadow-amber-500/20'
            : 'bg-slate-900/60 border-slate-800/80 hover:border-slate-700/80'
        }`}
      >
        <div className="flex items-center justify-between">
          ...
          <div>
            <span
              className={`text-xs font-bold block ${
                avgScore < 50 ? 'text-rose-400' : avgScore < 75 ? 'text-amber-400' : 'text-emerald-400'
              }`}
            >
              {avgScore < 50 ? 'Critical Defect' : avgScore < 75 ? 'Suboptimal' : 'Fast'}
            </span>
            <span className="text-[11px] text-slate-400">{stats.auditedLeads} audited sites</span>
          </div>
        </div>
        <div className="mt-2 text-xs text-slate-400 flex items-center justify-between">
      ...
      >
        ...
        <div className="mt-2 text-xs text-slate-400 flex items-center justify-between">
          ...
          <span className="text-[11px] text-emerald-400 group-hover:text-emerald-300">Send outreach →</span>
        </div>
      </button>
    </div>
  );
